...
export function showCountdown(number) {
  console.log(`[Countdown] Attempting to show: ${number}`);
  
  // Wait a bit to ensure brewing screen is fully rendered
  setTimeout(() => {
    const brewingScreen = document.getElementById("brewing-screen");
    if (!brewingScreen) {
      console.error("[Countdown] Brewing screen not found!");
      return;
    }
    ...
    // Set number and show with initial scale
